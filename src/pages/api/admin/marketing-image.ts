...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (action === 'list') {
    ...
    const [{ data: cats }, { data: prods }, { count: pendingA }, { count: approvedA }, { count: totalP }, { count: withMockup }, { count: withMockupB }, { count: pendingB }, { count: approvedB }, { count: pendingC }, { count: approvedC }] = await Promise.all([
      ...
      db.from('products').select('id', { count: 'exact', head: true }).not('mockup_b_url', 'is', null).eq('mockup_b_status', 'pending'),
      db.from('products').select('id', { count: 'exact', head: true }).not('mockup_b_url', 'is', null).eq('mockup_b_status', 'approved'),
      db.from('categories').select('id', { count: 'exact', head: true }).not('mockup_url', 'is', null).eq('mockup_status', 'pending'),
      db.from('categories').select('id', { count: 'exact', head: true }).not('mockup_url', 'is', null).eq('mockup_status', 'approved'),
    ]);
    ...
    return json({
      ...
      products: prods || [],
      stats: {
        // images, not products: a product can have two variants in review
        pending: (pendingA || 0) + (pendingB || 0) + (pendingC || 0),
        approved: (approvedA || 0) + (approvedB || 0) + (approvedC || 0),
