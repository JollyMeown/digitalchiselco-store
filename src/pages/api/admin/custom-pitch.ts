...
import { customQuoteReplyEmail } from '../../../lib/marketing-emails';
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (action === 'request_reply') {
      ...
      if (!id) return json({ error: 'id required' }, 400);
      if (message.length < 20) return json({ error: 'The reply is empty or too short.' }, 400);
      // A draft with blanks left in it must never reach a customer.
      if (/_{3,}|\$\s*_|\[price\]|\[days\]/i.test(message)) {
        return json({ error: 'The reply still has blanks in it (___). Fill in the price and delivery time first.' }, 400);
      }
      const { data: req } = await db.from('custom_design_requests').select('*').eq('id', id).maybeSingle();
