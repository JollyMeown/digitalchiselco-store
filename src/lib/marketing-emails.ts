  ...
  const body = `
    ...
    <div style="background:${CREAM};border:1px solid #E5DDD0;border-radius:10px;padding:18px 20px;margin:18px 0;">
      ...
      <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin-top:14px;">
        ...
        <tr><td style="font-size:13px;color:#555;padding:4px 0;">Delivery</td><td align="right" style="font-size:13px;color:${INK};padding:4px 0;">usually 1 to 2 days</td></tr>
...
export function customQuoteReplyEmail(d: {
  ...
  images?: string[];            // sample pictures the owner attached
}): Out {
  const subject = `Your custom relief quote (${d.ref})`;
  const pics = (d.images || []).slice(0, 8);
  // Two across, in rows: reads well at 544px and on a phone.
  const rows: string[] = [];
  for (let i = 0; i < pics.length; i += 2) {
    rows.push(`<tr>${pics.slice(i, i + 2).map((u) => `
      <td width="50%" style="padding:5px;vertical-align:top;">
        <a href="${esc(u)}"><img src="${esc(u)}" alt="sample" width="262" style="width:100%;max-width:262px;border-radius:8px;border:1px solid #E5DDD0;display:block;"></a>
      </td>`).join('')}${pics.slice(i, i + 2).length < 2 ? '<td width="50%"></td>' : ''}</tr>`);
  }
  const gallery = pics.length
    ? `<div style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:${BRONZE};font-weight:700;margin:6px 0 4px;">Samples</div>
       <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin:0 0 14px;">${rows.join('')}</table>`
    : '';
