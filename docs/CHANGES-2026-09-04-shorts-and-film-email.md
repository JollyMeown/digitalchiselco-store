...
## 4. YouTube mark on products (added, then removed the same day)

Built and deployed on 2026-09-05, then removed at the owner's request: the YouTube
marks and the "only designs with a YouTube film" switch live in **BRS** (design
grid badge + toolbar checkbox), not on the storefront. Migration 099 opened public
read of public `youtube_stats` rows; **100 reverts it** (admin-only again). The
storefront files were restored to their previous versions.

BRS side kept: every Cut renders two files, `FINAL.mp4` for the website (captions +
