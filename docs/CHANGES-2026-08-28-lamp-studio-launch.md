...
- **Mobile audit + header fix (commit a6c97ff)**: full phone-emulation pass (375x812) of the live playground - layout, all 4 wizard steps, export rows and the upsell modal are all clear on mobile (view on top 44vh, wizard below). One real defect found and fixed: the fixed 56px header overflowed (3-line title, overlapping pill, colliding buy link) - mobile breakpoint now uses an auto-height header, hides the pill, single-line title, right-aligned badge+link. launch.json lamp-studio entry moved to port 5111 (5107 was held by another session).
