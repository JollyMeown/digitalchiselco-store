...
const FIDELITY = `FIDELITY, THIS OUTRANKS EVERYTHING ELSE: image 1 is the exact job this photograph must show. Reproduce THAT alligator and no other: the same head turned to the right with the long snout and the visible row of teeth along the jaw, the same heavy brow and half lidded eye, the same ridged row of scutes running along the back, the same raised foreleg with clawed foot at the lower left, the same rumpled ground under the body, the same proportions and the same placement on the panel. The handwritten script words "The Carters" followed by the numerals "2026" are burned into the lower part of the panel exactly as in image 1: same wording, same flowing script, same size, same position. Do not redesign the animal, do not change its pose, do not add another animal, do not change or move the text.

THE BURN MAP, COPY IT EXACTLY FROM IMAGE 1: the laser has darkened ONE specific region and left the rest of the wood pale. That dark region is the background BEHIND the alligator. It begins below the pale textured strip across the very top of the panel, fills the whole area behind the raised back and the dorsal ridge, wraps over and behind the head, and runs right to the panel edge behind and above the snout. Everything else stays pale natural walnut: the top strip, the alligator itself, and the ground it stands on. The boundary between burned and pale follows the outline of the animal's back, head and snout precisely, as a scorched edge with a hair of soft feathering, never a cut-out or a sticker edge. The burned wood is deep roasted espresso brown with the grain still readable through it, not paint and not a flat fill.`

const SUBJECT = `THE OBJECT, PHOTOGRAPHED NOT RENDERED: one finished portrait panel of solid American black walnut, about 30 cm tall and 21 cm wide and 20 mm thick, with a plain square edge. The alligator is a genuine three dimensional carved relief standing proud of the panel, cut by a CNC router, with real carved depth of about 8 mm at the highest point. The background field AROUND the alligator has been LASER DARKENED to a deep roasted espresso brown, almost black in the deepest part, which is what makes the pale carved animal jump forward. The burn has a soft edge where it meets the carving, not a cut-out line. The name beneath is a fine laser burn, dark brown, slightly sunk into the surface.`;
...
const COMPOSITION = `COMPOSITION: the panel sits on the third, leaning back slightly against a warm off-white plastered wall, the plaster a soft neutral cream with no green or yellow cast, standing on a worn walnut bench. Diagonal, never square to frame. A soft out of focus near foreground edge of the bench. Background receding and quiet.`;
